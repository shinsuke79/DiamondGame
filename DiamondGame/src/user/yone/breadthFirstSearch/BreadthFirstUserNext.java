...
/**
 * 幅優先探索でNターン先までのユーザーが選択できる駒移動を予測しますが、<br>
 * 無駄な探索を独自の実装で省略しました。
...
public class BreadthFirstUserNext extends User {
	 ...
	 * 大きくすればパフォーマンスは上がるが探索の幅が狭まる
	 */
	private final double PERMIT_HN_COEFFICIENT = 0.97; // LIMIT:2なら0.95, LIMIT:3なら0.97がバランス良い

	public BreadthFirstUserNext(UserInfo userInfo) {
		super(userInfo);
		...
		while(true){
			...
			// その他のノードならノードを展開
				...
				for(Node child : children){
					// ノードを展開するが、現在のh(n)より残念な場合はOpenListに入れない
					if(child.hn >= (maxHn*PERMIT_HN_COEFFICIENT)){
						openList.add(child);
						if(child.hn >= maxHn){
							maxHn = child.hn;
						}
	...
	public static class BreadthFirstNextUserInfo extends UserInfo {

		public BreadthFirstNextUserInfo(String name) {
			super(name);
		}

		@Override
		public Class<? extends User> getUserClass() {
			return BreadthFirstUserNext.class;
		}
